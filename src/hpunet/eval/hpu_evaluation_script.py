...
from hpunet.metrics import binarize_logits
...
def generate_posterior_reconstructions(
    ...
    with torch.no_grad():
        for grader_idx in range(4):
            ...
            # Generate posterior reconstruction (mean of posterior)
            logits, _ = model(
                x=image,
...
def main():
    parser = argparse.ArgumentParser(description="HPU-Net Evaluation and Visualization")
    ...
    for example_idx, batch in enumerate(test_loader):
        ...
        create_visualization_grid_style1(
            ct_scan, np.array(grader_masks_np), reconstructions_np, samples_np, save_path_1
        )
        ...
        create_visualization_grid_style2(
            ct_scan, np.array(grader_masks_np), reconstructions_np, samples_np[:6], save_path_2
        )

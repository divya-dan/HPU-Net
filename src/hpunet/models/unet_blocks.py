...
class Down(nn.Module):
    """downsample with avg pool + conv block"""
...
class ResDown(nn.Module):
    """avg pool downsampling + residual stack"""
    def __init__(self, in_ch: int, out_ch: int, n_blocks: int = 3):
        super().__init__()
        self.pool = nn.AvgPool2d(2)  # avg-pool per paper
...
class ResUp(nn.Module):
    """nearest upsample + skip connection + residual stack"""
...
class UNetEncoder(nn.Module):
    """5-scale U-Net encoder with conv blocks"""
...
class UNetDecoder(nn.Module):
    """5-scale U-Net decoder with skip connections"""
